...
class Paths():
    ...
    path_ai_config  =   os.path.join(path,"ai")
...
class Sprites():
    ...
    # Sprite Images
    player_sprite_s     =   pg.image.load(os.path.join(Paths.path_spaceships,"player_sprite_static.png")).convert_alpha()
    player_sprite_k     =   pg.image.load(os.path.join(Paths.path_spaceships,"player_sprite_kinetic.png")).convert_alpha()

    player_plane_s      =   pg.image.load(os.path.join(Paths.path_spaceships,"ship2_static.png")).convert_alpha()
    player_plane_k      =   pg.image.load(os.path.join(Paths.path_spaceships,"ship2_moving.png")).convert_alpha()
...
class World():
    ...
    background      =   pg.image.load(os.path.join(Paths.path_background,"wallpaper_2_layer2.png"))
    background      =   pg.transform.scale(background,Setup.map_size)


    parallax        =   pg.image.load(os.path.join(Paths.path_background,"wallpaper_2.png")).convert()
    parallax        =   pg.transform.scale(parallax,Setup.map_size)
    ...
    def  render_map():
        ...
        #World.game_surface.fill((255,255,255))
...
class Level():
    ...
    Change difficulty change in the level_update() function.
    """
    #General
    ...
    def level_update():
        ...
        Level.enemy_shooting_speed    += 0   
        Level.enemy_field_of_view     += 0
    ...
    def level_reset():
        ...
        Level.enemies                 = 6   #amount of enemies on map
        Level.enemy_damage            = 0.5   #damage done by 1 enemy shot
        Level.enemy_shooting_speed    = 300  #shot speed in pixels per seconds
        Level.enemy_field_of_view     = 800   #in pixels
...
class Upgrades(pg.sprite.Sprite):
    ...
    def set_upgrades():
        ...
        Upgrades.regeneration_speed  = 500   #regenerating health points per second
...
class Mining_Reach(pg.sprite.Sprite):
    ...
    ressources with player. Moves with players position. Is transparent
    surface.
    """
...
class Rendering_Window(pg.sprite.Sprite):
    ...
    collisions with all entities. Game runs smoother when only enties in view
...
class Entity(pg.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.spawn_x = random.choice([
            random.randint(-10,-5),
            random.randint(Setup.map_size[0]+5,Setup.map_size[0]+10)
            ])

        self.spawn_y = random.choice([
            random.randint(-10,-5),
            random.randint(Setup.map_size[1]+5,Setup.map_size[1]+10)
            ])
...
class Enemy(pg.sprite.Sprite):
    ...
    def __init__(self,spawn_x=None,spawn_y=None):
        ...
        if spawn_x is None:
            spawn_x = random.randint(500,5500)
        if spawn_y is None:
            spawn_y = random.randint(500,5500)
...
class AI():
    ...
    class Opponent(pg.sprite.Sprite):
        ...
        def __init__(self):
            ...
            """
            This is the AI Entity class. It handles the mechanics and development of the NPC entity type: AI
            ...
            spawn_x         = 3000 #int(random.randint(Setup.map_size[0]/4,Setup.map_size[0]/4*3))
            spawn_y         = 3000 # int(random.randint(Setup.map_size[1]/4,Setup.map_size[1]/4*3))
            ...
            self.field_of_view          = (400,400) #how far can this monster look ahead?
        ...
        def regenerate(self,time):
            ...
            #if self.data_ai[8] < self.dash_capacity:
            #    self.data_ai[8] += (self.dash_reg_speed)*time
            pass
        ...
        def breaker(self):
            limit = 140   
            if self.mover[0] > limit:
                self.mover[0] -= 1
            if self.mover[1] > limit:
                self.mover[1] -= 1
            if self.mover[0] < -limit:
                self.mover[0] += 1
            if self.mover[1] < -limit:
        ...
        def keep_inbound(self):
            detected_belt = pg.sprite.spritecollide(self.sector, Sprites.asteroid_belt, False)
            

            for asteroid in detected_belt:
                vector = Maths.get_normalized_vector(self.rect.center,self.find_player)

                can_move= self.check_speed()
                if can_move is True:
        ...
        def up(self):
            ...
            self.is_moving = True
            self.data_ai[4] = 0
            if self.mover[1] > -Upgrades.moving_speed:
        ...
        def right(self):
            self.is_moving = True
            self.data_ai[4] = 270
            if self.mover[0] < Upgrades.moving_speed:
        ...
        def hunt(self):
            ...
            detected_targets += pg.sprite.spritecollide(self.sector, Sprites.asteroid_belt, False)

            Sprites.ai_list.remove(self)
            detected_targets += pg.sprite.spritecollide(self.sector, Sprites.ai_list, False)
            Sprites.ai_list.add(self)
            """
            for entity in detected_targets:
                ...
                if entity.rect.center[0] > self.rect.center[0]-20 and entity.rect.center[0] < self.rect.center[0]+20:
                    Enemy_shooting(self.shooting_direction(entity,axis='y'),(self.rect.center))

                elif entity.rect.center[1] > self.rect.center[1]-20 and entity.rect.center[1] < self.rect.center[1]+20:
                    Enemy_shooting(self.shooting_direction(entity,axis='x'),(self.rect.center))

                else: 
                    continue
            """
            if detected_targets:
                self.target_x = detected_targets[-1].rect.center[0]
                self.target_y = detected_targets[-1].rect.center[1]
    ...
    class Sector(pg.sprite.Sprite):
        def __init__(self,opponent):
            ...
            try:
                self.mining_reach   = pg.Surface(self.size).convert()
            
            except:
                self.mining_reach   = pg.Surface((1,1)).convert()
                #self.ship.kill()
                #self.kill()
                #AI.destroy_all = True
    ...
    @staticmethod
    def run(config_file,new_game):
        """
        runs the NEAT algorithm to train a neural network to play flappy spam_filter.
        ...
        winner = p.run(new_game.game_loop, 400)
        random.randint(0,1000)
        title = "best"+str(random.randint(0,1000))+".pickle"
        pickle.dump(winner,open(title, "wb"))
...
class Gui():
    ...
    class Messages():
        ...
        """
        Used to display tutorial instructions. Not working properly, to be fixed soon
        """
        ...
        texts = [
            ...
            'GATHER  AMMO  NEAR  PINK  ASTEROIDS', 
    ...
    def positioning(self):
        ...
        self.y_overheat = Setup.screen_y/5*2
...
class Controller():
    ...
    @staticmethod
    def movement_old(time):
        pressed_keys = pg.key.get_pressed()
        is_moving = False
        if pressed_keys[K_a]:
            is_moving = True
            if pressed_keys[K_w]:
                Player.camera[0] += Upgrades.moving_speed *   time * Player.diagonal_factor
            elif pressed_keys[K_s]:
                Player.camera[0] += Upgrades.moving_speed *   time * Player.diagonal_factor
            else:
                Player.camera[0]            += Upgrades.moving_speed *   time
                
        if pressed_keys[K_d]:
            is_moving = True
            if pressed_keys[K_w]:
                Player.camera[0] += Upgrades.moving_speed * (-1) *   time * Player.diagonal_factor
            elif pressed_keys[K_s]:
                Player.camera[0] += Upgrades.moving_speed * (-1) *   time * Player.diagonal_factor
            else:
                Player.camera[0] += Upgrades.moving_speed * (-1) *   time

        if pressed_keys[K_w]:
            is_moving = True
            if pressed_keys[K_a]:
                Player.camera[1] += Upgrades.moving_speed *   time * Player.diagonal_factor
            elif pressed_keys[K_d]:
                Player.camera[1] += Upgrades.moving_speed *   time * Player.diagonal_factor
            else:
                Player.camera[1] += Upgrades.moving_speed *   time

        if pressed_keys[K_s]:
            is_moving = True
            if pressed_keys[K_a]:
                Player.camera[1] += Upgrades.moving_speed * (-1) *   time * Player.diagonal_factor
            elif pressed_keys[K_d]:
                Player.camera[1] += Upgrades.moving_speed * (-1) *   time * Player.diagonal_factor
            else: 
                Player.camera[1] += Upgrades.moving_speed * (-1) *   time
        ...
        for event in pg.event.get():
                ...
                if event.type == KEYDOWN:
                    if event.key == K_ESCAPE:
                        main.Flow.get_back_to_menu(Player.data_player[6])
...
class Collisions():
    ...
    def enemy_fire(player):
        ...
        for shot in Sprites.enemy_shot_list:
            ...
            #enemy_on_enemy_list         =   pg.sprite.spritecollide(shot, Sprites.enemy_list,False)
            #for enemy in enemy_on_enemy_list:
            #    enemy.health            -=  Level.enemy_damage 
                #Explosion.explode(shot.rect.x,shot.rect.y)
    ...
    def enemy_hit(player):
        ...
        for ai in ai_player_hit:
            ...
            #Explosion.explode(player.rect.center[0],player.rect.center[1])
            #Controller.get_pushed((ai.mover[0],ai.mover[1]))
        """
        for ai in Sprites.ai_list:
    ...
    def ai_offmap():
        for ai in Sprites.ai_list:
            if ai.rect.x < 100 or ai.rect.x > Setup.map_size[0]-100 or ai.rect.y < 100 or ai.rect.y > Setup.map_size[1]-100:
...
class Game():
    ...
    def game_loop(self,genomes, config):
        ...
        # opponent object that uses that network to play
        nets = []
        opponents = []
        ge = []
        ...
        for genome_id,genome in genomes:
            ...
            nets.append(net)
            opponents.append(AI.Opponent())
            ge.append(genome)

        Enemy(spawn_x=3200,spawn_y=3200)
        Enemy(spawn_x=2800,spawn_y=2800)

        while len(opponents) > 0:

            if AI.destroy_all is True:
                for opponent in opponents:
                    nets.pop(opponents.index(opponent))
                    ge.pop(opponents.index(opponent))
                    opponents.pop(opponents.index(opponent))
                AI.destroy_all = False


            for x, opponent in enumerate(opponents):  # give each opponent a fitness of 0.1 for each frame it stays alive
                ge[x].fitness += 0.1
                
                target_list = opponent.look()
                # send opponent location of self and all detected targets in viewing field
                for target in target_list:
                    output = nets[opponents.index(opponent)].activate((
                    ...
                    if output[0] > 0.5:  # we use a tanh activation function so result will be between -1 and 1. if over 0.5 move
                        opponent.up()
                    if output[1] > 0.5:
                        opponent.down()
                    if output[2] > 0.5:
            ...
            render_list = pg.sprite.spritecollide(self.rendering_window, Sprites.all_sprites_list, False) #checks which sprites are actually in view
            ...
            #fitness modifications of genomes

            for opponent in opponents:
                belt_collision = pg.sprite.spritecollide(opponent,Sprites.asteroid_belt,False)
                if len(belt_collision) > 0:
                        ge[opponents.index(opponent)].fitness -= 10000
                        nets.pop(opponents.index(opponent))
                        ge.pop(opponents.index(opponent))
                        opponents.pop(opponents.index(opponent))
                        opponent.kill()
            ...
            for opponent in opponents:
                shot_hit_list = pg.sprite.spritecollide(opponent,Sprites.enemy_shot_list,True)
                if len(shot_hit_list) > 0:
                    ge[opponents.index(opponent)].fitness -= 10
                    nets.pop(opponents.index(opponent))
                    ge.pop(opponents.index(opponent))
                    opponents.pop(opponents.index(opponent))
                    opponent.kill()
            ...
            for opponent in opponents:
                ai_on_enemy_list = pg.sprite.spritecollide(opponent, Sprites.enemy_list, False)
                if len(ai_on_enemy_list) > 0:
                    ge[opponents.index(opponent)].fitness -= 20
                    nets.pop(opponents.index(opponent))
                    ge.pop(opponents.index(opponent))
                    opponents.pop(opponents.index(opponent))
                    opponent.kill()
            ...
            for opponent in opponents:
                if opponent.data_ai[5] <= 60 :
                    ge[opponents.index(opponent)].fitness -= 5
                    nets.pop(opponents.index(opponent))
                    ge.pop(opponents.index(opponent))
                    opponents.pop(opponents.index(opponent))
                    opponent.kill()
            ...
            for opponent in opponents:
                if opponent.mover[0] == 0 and opponent.mover[1] == 0:
                    ge[opponents.index(opponent)].fitness -= 4
                if opponent.mover[0] < -30 or opponent.mover[0] >30:
                    ge[opponents.index(opponent)].fitness += 0.1 
                if opponent.mover[1] < -30 or opponent.mover[1] >30:
                    ge[opponents.index(opponent)].fitness += 0.1
            ...
            for opponent in opponents:
                if opponent.data_ai[1] < 200 or opponent.data_ai[2] < 200:
                    ge[opponents.index(opponent)].fitness -= 100000
                    nets.pop(opponents.index(opponent))
                    ge.pop(opponents.index(opponent))
                    opponents.pop(opponents.index(opponent))
                    opponent.kill()
            ...
            for opponent in opponents:
                if opponent.data_ai[1] > Setup.map_size[0]-200 or opponent.data_ai[2] > Setup.map_size[1]-200:
                    ge[opponents.index(opponent)].fitness -= 100000
                    nets.pop(opponents.index(opponent))
                    ge.pop(opponents.index(opponent))
                    opponents.pop(opponents.index(opponent))
    ...
    def new_game():
        ...
        config_path = os.path.join(local_dir,"ai", 'config_ai.txt')
        AI.run(config_path,new_game)
